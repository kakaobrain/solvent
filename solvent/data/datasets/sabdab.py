...
def load_sabdab(data_dir):
    cache_path = os.path.join(data_dir, 'chain_data_cache.json')
    with open(cache_path, 'r') as f:
        data = json.load(f)

    dataset_dicts = []
    for full_name, _ in data.items():
        records = {}
        
        comp = full_name.split('_')
        if len(comp) == 2:
            file_id, chain_id = comp
        else:
            file_id = comp[0]
            chain_id = 'H'

        records['data_name'] = 'sabdab'
...
def register_sabdab(name, data_dir):
    DatasetCatalog.register(name, lambda: load_sabdab(data_dir))
    MetadataCatalog.get(name).set(
        evaluator_type="antibody_folding",
    )
